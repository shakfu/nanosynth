...
class server_arguments {
    ...
    server_arguments() = default;  // nanosynth: for embedded initialization
...
public:
    ...
    /** nanosynth: initialize with direct field values (no argc/argv parsing) */
    static server_arguments& initialize_embedded() {
        instance_.reset(new server_arguments());
        return *instance_;
    }
